"""Core nitrogen model functions for data standardization and Layer 1 calculations (app).

- Standardize sanitation table
- Apply interventions (OD→septic, pit→septic)
- Compute Layer 1 nitrogen_load = Pop × Protein_per_capita × Protein_to_N_conversion × (1 − η) × 365
"""
...
def compute_layer1_nitrogen_loads(df: pd.DataFrame, protein_per_capita: float, protein_to_nitrogen_conversion: float) -> pd.DataFrame:
    df = df.copy()
    # Annual nitrogen load calculation: population × protein_per_capita × protein_to_nitrogen_conversion × (1 - η) × 365
    df['nitrogen_load'] = (
        df['household_population'] * 
        protein_per_capita * 
        protein_to_nitrogen_conversion * 
        (1 - df['pathogen_containment_efficiency']) * 
        365
    )
    df[['id', 'lat', 'long', 'toilet_category_id', 'nitrogen_load']].to_csv(config.NET_NITROGEN_LOAD_PATH, index=False)
    logging.info(f"Saved net nitrogen load to {config.NET_NITROGEN_LOAD_PATH} ({len(df)} records)")
    return df
...
def build_or_load_household_tables(scenario: Dict[str, Any]) -> pd.DataFrame:
    ...
    protein_per_capita = scenario.get('protein_per_capita_override') or config.PROTEIN_PER_CAPITA_DEFAULT
    protein_to_nitrogen_conversion = scenario.get('protein_to_nitrogen_conversion_override') or config.PROTEIN_TO_NITROGEN_CONVERSION
    df_with_loads = compute_layer1_nitrogen_loads(df_with_interventions, protein_per_capita, protein_to_nitrogen_conversion)
